...
const followButtonHTML = `        {author && currentUserId && author.uid !== currentUserId && onFollowToggle && (
          <button
            onClick={(e) => {
              e.stopPropagation();
...
// 1. Remove it from the bottom section
content = content.replace(/\{author && currentUserId && author\.uid !== currentUserId && onFollowToggle && \([\s\S]*?<\/button>\s*\)\}/, '');

// 2. Add it to the image section
content = content.replace(
  /<\/div>\s*<\/div>\s*<div className="p-3 flex justify-between items-center bg-zinc-50 dark:bg-zinc-800\/50">/,
